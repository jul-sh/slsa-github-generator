...
Before using the Maven publish action, you should have a workflow that invokes the [Maven builder](https://github.com/jul-sh/slsa-github-generator/blob/main/internal/builders/maven/README.md). It will look something like this:
...
jobs:
  build:
    permissions:
      ...
      contents: read
      actions: read
    uses: jul-sh/slsa-github-generator/.github/workflows/builder_maven_slsa3.yml@v1.10.0
    with:
...
publish:
  ...
  steps:
    - name: publish
      id: publish
      uses: jul-sh/slsa-github-generator/actions/maven/publish@v1.10.0
      with:
